...
async function run() {
  try {
    ...
    // Verify row counts
    const cc = await db.query('SELECT COUNT(*) FROM contacts');
    const dc = await db.query('SELECT COUNT(*) FROM deals');
    const jc = await db.query('SELECT COUNT(*) FROM journeys');
    console.log('=== Sync complete ===');
    console.log('Contacts in DB:', cc.rows[0].count);
    console.log('Deals in DB:', dc.rows[0].count);
    console.log('Journey events in DB:', jc.rows[0].count);
  } catch (err) {
